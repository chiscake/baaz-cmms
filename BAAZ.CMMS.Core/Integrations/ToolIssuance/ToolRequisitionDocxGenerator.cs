...
public sealed class ToolRequisitionDocxGenerator : IToolRequisitionDocxGenerator
{
    ...
    private const int CellPaddingTwips = 100;
    ...
    private static void AppendApprovalHeader(Body body, int year)
    {
        ...
        AppendParagraph(body, "Начальник отдела выдачи инструмента", alignment: JustificationValues.Right);
    ...
    private static void AppendTitle(Body body, string requisitionNumber, DateTime documentDate)
    {
        ...
        AppendParagraph(body, $"ЗАЯВКА № {requisitionNumber}", bold: true, fontSizeHalfPoints: 28,
            alignment: JustificationValues.Center);
        AppendParagraph(body, "на выдачу инструмента со склада", bold: true, fontSizeHalfPoints: 28,
            alignment: JustificationValues.Center);
    ...
    private static void AppendLinesTable(Body body, IReadOnlyList<ToolRequisitionLine> lines)
    {
        ...
        foreach (var line in lines)
        {
            ...
            row.Append(CreateTableCell(line.InventoryNumber ?? string.Empty, fontSizeHalfPoints: 20));
            row.Append(CreateTableCell(line.Name, fontSizeHalfPoints: 20));
            row.Append(CreateTableCell(line.Quantity.ToString(RuCulture), fontSizeHalfPoints: 20, alignment: JustificationValues.Center));
    ...
    private static void AppendSignatures(Body body, string authorFullName, string technicianFullName)
    {
        ...
        AppendSignatureRow(table, "Выдал (Кладовщик):", string.Empty);
        AppendSignatureRow(table, "Получил (Исполнитель):", technicianFullName);
